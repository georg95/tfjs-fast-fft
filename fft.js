...
function fftWebGPU({ backend, inputs: { input } }, inverse) {
    ...
    const { real: realIn, imag: imagIn } = backend.tensorMap.get(input.dataId).complexTensorInfos
    let currentTensor = backend.runWebGPUProgram(
        new BitReverseGPU(batch, innerDim, true), [realIn, imagIn], 'float32')
    for (let len = 1; len * 2 < innerDim; len *= 2) {
        let prevTensor = currentTensor
        currentTensor = backend.runWebGPUProgram(
            new CooleyTukeyStepGPU(batch, innerDim, len, 'complex', inverse), [currentTensor], 'float32')
        backend.disposeData(prevTensor.dataId)
    }
    const real = backend.runWebGPUProgram(
        new CooleyTukeyStepGPU(batch, innerDim, innerDim / 2, 'real', inverse), [currentTensor], 'float32')
    const imag = backend.runWebGPUProgram(
        new CooleyTukeyStepGPU(batch, innerDim, innerDim / 2, 'imag', inverse), [currentTensor], 'float32')
...
tf.registerKernel({
    ...
    kernelFunc: (arg) => {
        ...
        if ((innerDim & (innerDim - 1)) !== 0) {
            console.warn('Using slow FFT implementation, because tensor size is not power of 2')
...
tf.registerKernel({
    ...
    kernelFunc: (arg) => {
        const innerDim = arg.inputs.input.shape[arg.inputs.input.shape.length - 1]
        if ((innerDim & (innerDim - 1)) !== 0) {
            console.warn('Using slow IFFT implementation, because tensor size is not power of 2')
...
class CooleyTukeyStepGPU {
    ...
    constructor(batch, innerDim, len, output='complex', inverse=false) {
        ...
        this.dispatchLayout = flatDispatchLayout(this.outputShape)
        this.dispatch = computeDispatch(
            this.dispatchLayout, this.outputShape, this.workgroupSize, [output === 'complex' ? 2 : 1, 1, 1])
    }
    getUserCode() {
        ...
        return `fn main(index: i32) {
            ...
            let outIndexReal = ${output === 'real' ? `index` : `batch * ${innerDim * 2} + i` };
            let outIndexImag = ${output === 'imag' ? `index` : `outIndexReal + ${innerDim}` };
            let isHigh = (i % (${len} * 2)) / ${len};
            let highSign = (1 - isHigh * 2);
            let baseIndex = i - isHigh * ${len};
            let t = ${inverse ? -Math.PI / len : Math.PI / len} * f32(i % ${len});
            let a = cos(t);
            let b = sin(-t);
            ...
            ${output !== 'imag' ? `
                ...
                setOutputAtIndex(outIndexReal, ${inverse && output === 'real' ? `outpR / f32(${innerDim})` : 'outpR'});
                ` : ''}
            ${output !== 'real' ? `
                let evenK_im = getX(batch, baseIndex + ${innerDim});
                let outpI = (evenK_im + (oddK_re * b + oddK_im * a) * f32(highSign));
                setOutputAtIndex(outIndexImag, ${inverse && output === 'imag' ? `outpI / f32(${innerDim})` : 'outpI'});
                ` : ''}
        }`
    }
}
...
tf.registerKernel({
    ...
    kernelFunc: ({ backend, inputs: { input } }) => {
        ...
        let currentTensor = backend.runWebGPUProgram(
            new BitReverseGPU(batch, innerDim), [input], 'float32')
        for (let len = 1; len < innerDim; len *= 2) {
            let prevTensor = currentTensor
            currentTensor = backend.runWebGPUProgram(
                new CooleyTukeyStepGPU(batch, innerDim, len), [currentTensor], 'float32')
            backend.disposeData(prevTensor.dataId)
        }
        const real = backend.runWebGPUProgram(
            new RfftReassembleGPU(batch, innerDim, 'real'), [currentTensor], 'float32')
        const imag = backend.runWebGPUProgram(
            new RfftReassembleGPU(batch, innerDim, 'imag'), [currentTensor], 'float32')
        const complex = tf.engine().runKernel('Complex', { real, imag })
        backend.disposeData(currentTensor.dataId)
        backend.disposeData(real.dataId)
        backend.disposeData(imag.dataId)
...
function fftWebGL({ backend, inputs: { input } }, inverse) {
    const innerDim = input.shape[input.shape.length - 1]
    const batch = tf.util.sizeFromShape(input.shape) / innerDim
    const { real: realIn, imag: imagIn } = backend.texData.get(input.dataId).complexTensorInfos
    let currentTensor = backend.runWebGLProgram(
        new BitReverse(batch, innerDim, true), [realIn, imagIn], 'float32')
    for (let len = 1; len * 2 < innerDim; len *= 2) {
        let prevTensor = currentTensor
        currentTensor = backend.runWebGLProgram(
            new CooleyTukeyStep(batch, innerDim, len, 'complex', inverse), [currentTensor], 'float32')
        backend.disposeIntermediateTensorInfo(prevTensor)
    }
    const real = backend.runWebGLProgram(
        new CooleyTukeyStep(batch, innerDim, innerDim / 2, 'real', inverse), [currentTensor], 'float32')
    const imag = backend.runWebGLProgram(
        new CooleyTukeyStep(batch, innerDim, innerDim / 2, 'imag', inverse), [currentTensor], 'float32')
    const complex = tf.engine().runKernel('Complex', { real, imag })
...
class CooleyTukeyStep {
    ...
    constructor(batch, innerDim, len, output='complex', inverse) {
        ...
        this.userCode = `void main() {
            ...
            int batch = coords[0];
            int i = ${output === 'imag' ? `coords[1] + ${innerDim}` : 'coords[1]'};
            int k = i % ${innerDim};
            int isHigh = (k % ${len * 2}) / ${len};
            int highSign = (1 - isHigh * 2);
            int baseIndex = k - isHigh * ${len};
            float t = ${inverse ? -Math.PI / len : Math.PI / len} * float(k % ${len});
            float a = cos(t);
            float b = sin(-t);
            ...
            if (i < ${innerDim}) { // real
                ...
                setOutput(${inverse && output === 'real' ? `outp / float(${innerDim})` : 'outp'});
            } else { // imaginary
                float evenK_im = getX(batch, baseIndex + ${innerDim});
                float outp = evenK_im + (oddK_re * b + oddK_im * a) * float(highSign);
                setOutput(${inverse && output === 'imag' ? `outp / float(${innerDim})` : 'outp'});
            }
        }`;
    }
}
    ...
    constructor(batch, innerDim, readComplex=false) {
        ...
        this.userCode = `void main() {
            ...
            if (coords[1] < ${innerDim}) {
                setOutput(${readComplex ? `getReal(coords[0], k)` : `getX(coords[0], 2 * k)`});
            } else {
                setOutput(${readComplex ? `getImag(coords[0], k)` : `getX(coords[0], 2 * (k % ${innerDim}) + 1)`});
            }
        }`
    }
}
...
tf.registerKernel({
    ...
    kernelFunc: ({ backend, inputs: { input } }) => {
        ...
        const batch = tf.util.sizeFromShape(input.shape) / innerDim / 2
        let currentTensor = backend.runWebGLProgram(
            new BitReverse(batch, innerDim), [input], 'float32')
        for (let len = 1; len < innerDim; len *= 2) {
            let prevTensor = currentTensor
            currentTensor = backend.runWebGLProgram(
                new CooleyTukeyStep(batch, innerDim, len), [currentTensor], 'float32')
            backend.disposeIntermediateTensorInfo(prevTensor)
        }
        const real = backend.runWebGLProgram(
            new RfftReassemble(batch, innerDim, 'real'), [currentTensor], 'float32')
        const imag = backend.runWebGLProgram(
            new RfftReassemble(batch, innerDim, 'imag'), [currentTensor], 'float32')
        const complex = tf.engine().runKernel('Complex', { real, imag })
        backend.disposeIntermediateTensorInfo(currentTensor)
        backend.disposeIntermediateTensorInfo(real)
        backend.disposeIntermediateTensorInfo(imag)
